...
export async function loadSkills(state: SkillsState, options?: LoadSkillsOptions) {
  ...
  if (!state.client) {
    state.skillsError = "Client not initialized";
    state.skillsLoading = false;
    return;
  }
  if (!state.connected) {
    state.skillsError = "Not connected to gateway";
    state.skillsLoading = false;
    return;
  }
  ...
  try {
    ...
    else state.skillsError = "No skills report received";
  } catch (err) {
...
export async function updateSkillEnabled(state: SkillsState, skillKey: string, enabled: boolean) {
  if (!state.client || !state.connected) {
    const message = !state.client ? "Client not initialized" : "Not connected to gateway";
